...
# Reads client_profile.json and serp_results_top10.csv, classifies domains,
# flags competitors/allies/taboos, and writes a one-page Markdown summary.
...
INPUT_TOP10 = "serp_results_top10.csv"   # produced in Lesson 3
...
# lightweight language/region checks (optional placeholders you can expand later)
def language_ok(_row_langs, _allowed_langs):
    # If you later add a Language column, use it here. For now always True.
    return True

def region_ok(_row_region, _allowed_regions):
    # If you later add a Country/Region column, use it here. For now always True.
    return True

# Simple heuristic to surface scare/stigma language in Titles/Descriptions
SCARE_TERMS = [
...
# Ensure we have Domain; if not, try to derive from URL
...
# Scare/Stigma heuristics from title/description
if title_col:
    df["TitleScaremongering"] = df[title_col].apply(has_scare_language)
else:
    df["TitleScaremongering"] = False

if desc_col:
    df["DescScaremongering"] = df[desc_col].apply(has_scare_language)
else:
    df["DescScaremongering"] = False

df["AnyScaremongering"] = df["TitleScaremongering"] | df["DescScaremongering"]

# simple region/language checks (placeholders for later)
df["LangOK"] = True if not languages else True
df["RegionOK"] = True if not regions else True
